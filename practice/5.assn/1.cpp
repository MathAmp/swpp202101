...
public:
  PreservedAnalyses run(Function &F, FunctionAnalysisManager &FAM) {\
    // Do not TOO many use nested pair. Make another struct or class.
	...
	// Elements of inv_dom differ only in order from dom's. One is unnecessary.
	for(BasicBlock &BBstart : F){
  		BranchInst *TI = dyn_cast<BranchInst>(BBstart.getTerminator());
  		// Why left empty condition? Just use if (TI) {}
      		if (!TI) {
      		} else {
	...
	for (auto &BB : F) {
		for (auto &I : BB) {
			...
			case Instruction::ICmp:{
				...
				// Predicate Num is hardcoded. Use internal constant like next
